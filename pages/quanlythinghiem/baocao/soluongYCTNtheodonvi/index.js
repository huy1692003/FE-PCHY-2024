...
const BaoCao_YCTN_TheoDonViThucHien = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const result = await BAO_CAO_Service.get_SL_QLTN_theoDonViThucHien({
        startDate: start,
        endDate: end,
      });
      setData(result);
  ...
  const DownloadExcel = async () => {
    try {
      ...
      worksheet.getCell(`A${1}`).value =
        "BÁO CÁO SỐ LƯỢNG THÍ NGHIỆM THEO ĐƠN VỊ THỰC HIỆN";
      ...
      data.forEach((item, index) => {
        ...
        worksheet.getCell(`B${rowIndex}`).value = item.ten_don_vi;
      ...
      const filename = `ExportSLTNTDV_${timestamp}.xlsx`; // Tên file với timestamp
...
export default BaoCao_YCTN_TheoDonViThucHien;
