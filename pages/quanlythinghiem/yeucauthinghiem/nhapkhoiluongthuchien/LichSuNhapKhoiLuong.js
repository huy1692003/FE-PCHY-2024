...
const LichSuNhapKhoiLuong = ({ nextStep, thongTinYCTN }) => {
  ...
  return (
    <div>
      {Array.from(new Set(arrThietbiBD.map((device) => device.trang_thai)))
        .sort((a, b) => a - b)
        .map((trangThai, index) => (
          <Panel
            header={`Thiết bị ${
      ...
      <div className="flex justify-content-end gap-2 mt-3">
        ...
        <Button
          ...
          onClick={() => {
            router.push(
              `/quanlythinghiem/yeucauthinghiem/nhapkhoiluongphatsinh?code=${thongTinYCTN?.ma_yctn}`
            );
          }}
        />
        ...
        <Button
          ...
          onClick={() => {
            router.push(
              `/quanlythinghiem/yeucauthinghiem/khaosatphuongan?code=${thongTinYCTN?.ma_yctn}`
            );
          }}
        />
      </div>
